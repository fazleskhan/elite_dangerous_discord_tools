...
def test_bulk_load_algo_create_and_neighbor_payload_handling() -> None:
    ...
    assert algo._neighbor_as_system_info(
        {"name": "Sol", "coords": {"x": 1, "y": 2, "z": 3}}
    ) == {
        "name": "Sol",
        "coords": {"x": 1, "y": 2, "z": 3},
    }
...
def test_bulk_load_algo_loads_neighbors_and_respects_limits(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    graph = {
        "Sol": [
            {"name": "Alpha", "coords": {"x": 1, "y": 2, "z": 3}},
            {"name": "Beta"},
        ],
    ...
    assert algo.load(["Sol", "Sol", " "], 3, progress.append) == [
        "Sol",
        "Alpha",
        "Beta",
    ]
...
def test_bulk_load_algo_physical_core_count_fallback(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setattr(
        ed_bulk_load_algo.psutil,
        "cpu_count",
        lambda logical=False: None if not logical else 8,
    )
