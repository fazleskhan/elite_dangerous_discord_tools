...
def test_import_tinydb_loads_json_files_and_inserts(tmp_path, monkeypatch):
    class FakeTinyDB:
        def __init__(self):
            self.inserted = []
...
def test_import_tinydb_raises_for_missing_dir(tmp_path, monkeypatch):
    missing_dir = tmp_path / "missing"
    monkeypatch.setattr(
        sys, "argv", ["import_tinydb.py", "--import-dir", str(missing_dir)]
    )
